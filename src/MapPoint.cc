...
void MapPoint::PreSave(set<KeyFrame*>& spKF,set<MapPoint*>& spMP)
{
    ...
    std::map<KeyFrame*,std::tuple<int,int> > tmp_Observations(mObservations);

    // Save the id and position in each KF who view it
    for(std::map<KeyFrame*,std::tuple<int,int> >::const_iterator it = tmp_Observations.begin(), end = tmp_Observations.end(); it != end; ++it)
    {
