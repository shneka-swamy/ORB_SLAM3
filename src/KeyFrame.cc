...
KeyFrame::KeyFrame():
        ...
        N(0), mvKeys(), mvKeysUn(), mvuRight(),
        mvDepth(), mnScaleLevels(0), mfScaleFactor(0), mfLogScaleFactor(0), mvScaleFactors(0), mvLevelSigma2(0), mvInvLevelSigma2(0),
        mnMinX(0), mnMinY(0), mnMaxX(0), mnMaxY(0), mPrevKF(nullptr), mNextKF(nullptr),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnNumberOfOpt(0),
    mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0), mnRelocWords(0), mnPlaceRecognitionQuery(0), mnPlaceRecognitionWords(0), mPlaceRecognitionScore(0), mbCurrentPlaceRecognition(false),
    mnBAGlobalForKF(0), mnMergeCorrectedForKF(0), mnBALocalForMerge(0), fx(F.fx), fy(F.fy), cx(F.cx),
    cy(F.cy), invfx(F.invfx), invfy(F.invfy), mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth),
    mDistCoef(F.mDistCoef), N(F.N), mvKeys(F.mvKeys),
    ...
    mfScaleFactor(F.mfScaleFactor), mfLogScaleFactor(F.mfLogScaleFactor), mvScaleFactors(F.mvScaleFactors), mvLevelSigma2(F.mvLevelSigma2),
    mvInvLevelSigma2(F.mvInvLevelSigma2), mnMinX(F.mnMinX), mnMinY(F.mnMinY), mnMaxX(F.mnMaxX), mnMaxY(F.mnMaxY),
    mPrevKF(nullptr), mNextKF(nullptr), mpImuPreintegrated(F.mpImuPreintegrated),
    mImuCalib(F.mImuCalib), mNameFile(F.mNameFile), mnDataset(F.mnDataset), mbHasVelocity(false), mTlr(F.GetRelativePoseTlr()), mTrl(F.GetRelativePoseTrl()),
    mvpMapPoints(F.mvpMapPoints), mpKeyFrameDB(pKFDB), mpORBvocabulary(F.mpORBvocabulary), mbFirstConnection(true), mpParent(nullptr), mbNotErase(false), mbToBeErased(false),
    mbBad(false), mHalfBaseline(F.mb/2),
    mpMap(pMap),mK_(F.mK_), mpCamera(F.mpCamera),
