...
int main(int argc, char **argv)
{
    ...
    ORB_SLAM3::System SLAM(argv[1],argv[2],ORB_SLAM3::System::IMU_MONOCULAR, false, std::string(), 0, file_name);
    float imageScale = SLAM.GetImageScale();

    std::fstream track_stream("tracking_pose.txt", std::ios::out);
    ...
    for(int ni=0; ni<nImages; ni++, proccIm++)
    {
      ...
      track_stream << setprecision(6) << 1e9*tframe << " " << setprecision(9) << pos.translation().x() << " " << pos.translation().y() << " " << pos.translation().z() << " "
              << pos.unit_quaternion().x() << " " << pos.unit_quaternion().y() << " " << pos.unit_quaternion().z() << " " << pos.unit_quaternion().w() << std::endl;
